    ...
    def scan_hosts(self) -> list:
        ...
        for h in self.hosts:
            ...
            for data_item in result['data']:
                ...
                ports.append(p)
            try:
                shodan_hosts.append(ShodanHostResult(result["ip_str"], ports, result['hostnames']))
            except ValueError as v:
                rootLogger.error(f"ValueError occured while creating new host result: {v}")
...
if __name__ == "__main__":
    print(ascii)
    parser = ArgumentParser(description="ShodanAutomator automates the search of hosts from an input list. More to come.")
    parser.add_argument("API_KEY", type=str, help="Submit your Shodan API key. This is required.")
    parser.add_argument("--host", type=str, help="Input a single host")
    parser.add_argument("--host-file", type=str, help="Input a file with hosts. Each on a new line.")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose ouptput (debug logging)")
